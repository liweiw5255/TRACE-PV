...
void launchKernel(double const* S_rated, double* thermal_loss) {
    ...
    int num_blocks_avg, num_blocks_a2s, num_blocks_mean, num_blocks_adjust, num_blocks_thermal;

    cudaDeviceProp deviceProp;
    CUDA_CHECK(cudaGetDeviceProperties(&deviceProp, 0)); // Assuming you're using the first GPU
    ...
    std::cout << "numThreads: " << maxThreadsPerBlock << " gridSize: " << deviceProp.maxGridSize[0] << " " << deviceProp.maxGridSize[1] << " " << deviceProp.maxGridSize[2] << " " 
              << " Requested Memory: " << requestedSize / (1024 * 1024) << " MB, Free memory: " << freeMem / (1024 * 1024) << " MB, Total memory: " << totalMem / (1024 * 1024) << " MB" << std::endl;
